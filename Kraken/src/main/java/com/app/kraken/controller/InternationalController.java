...
public class InternationalController {
	@Autowired
	IInternationalService service;
	
  @PostMapping("/domestic/register")
	public ResponseEntity<Domestic> register(@RequestBody International domestic)
	{
	  International domestic1 = service.registerInternational(domestic);		
		if(domestic1!=null)
		{
			return new ResponseEntity<International>(domestic1, HttpStatus.CREATED);
		}
		return new ResponseEntity<International>(HttpStatus.BAD_REQUEST);
	}
 
	@PutMapping("/domestic/update/{modelno}")
	public ResponseEntity<International> updateDining(@PathVariable long modelno,@RequestBody International domestic)
	{
		long status=0;
			status = service.updateInternational(domestic, modelno);
			International domestic1=service.getInternational(modelno);
			if(status!=0)
			{
				return new ResponseEntity<International>(domestic1, HttpStatus.CREATED);
			}
